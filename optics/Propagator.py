...
class Fres_Prop(nn.Module):
    ...
    def create_spatial_grid(self, H, W, dx, dy):
        ...
        """
        x = torch.linspace(-H * dx / 2, H * dx / 2, H, device=self.device)
        y = torch.linspace(-W * dx / 2, W * dx / 2, W, device=self.device)
...
def FraunhoferProp(field: ElectricField, z, padding=True):
    ...
    if len(field.wavelengths) != 1:
        raise Exception("Please use for loop outside the function to process multiple wavelength")

    wavelength = field.wavelengths[0]
    # Computational fourier optics. Chapter 5, section 5.5.
    # obs sample interval
    dx_obs, dy_obs = wavelength * z / Himg / field.spacing[0], wavelength * z / Himg / field.spacing[1]

    x = torch.linspace(-dx_obs * Himg / 2, dx_obs * Himg / 2, Himg, dtype=field.spacing[0].dtype)
    y = torch.linspace(-dy_obs * Wimg / 2, dy_obs * Wimg / 2, Wimg, dtype=field.spacing[1].dtype)
    X, Y = torch.meshgrid(x, y, indexing='ij')
    R = torch.sqrt(X**2 + Y**2).to(field.device)

    k = 2 * torch.pi / wavelength

    c = 1 / (1j *wavelength * z) * torch.exp(1j * k / (2 * z) * R**2)

    field_data = c * dx_obs * dy_obs * ifftshift(fft2(fftshift(padded_field)))
