...
class DDPTrainer:
    ...
    def __init__(self, 
        ...
        if self.network_config['net'] == 'unet':
            ...
            self.network = NAFNet(inp_channels=3, out_channels=3)
    ...
    def _run_batch(self, inputs, validate=False):
        ...
        loss = 0.3 * self.L1(inputs, x_output) + 0.7 * self.SSIMLoss(x_output, inputs) #+ 0.5 * self.PSNRLoss(x_output, inputs)#+ 0.5 * self.PerceptualLoss(x_output, inputs) #+ 0.1 * self.SSIMLoss(x_output, inputs)
        ...
        #  =====================Train and validate============================
            ...
            with tqdm(train_dataloader, unit='batch') as tepoch:
                for itr, data in enumerate(tepoch, 0):
                    ...
                    if iters % 500 == 0 and self.verbose:
